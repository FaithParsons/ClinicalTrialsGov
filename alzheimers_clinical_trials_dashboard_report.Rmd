...
Interactive lineplot was created for those currently recruiting interventional phase 3 trials in the US with available age elligibility creteria. A line in a plot represents a study, x-coordinates of a line (or length) represent study age elligibility, and y-coordinate of a line corresponds to a number of subjects enrolled in a study.   
...
Age Eligibility plot (**Figure 4**) for currently recruiting interventional phase 3 trials, how long each trial is expected to run, and the age range of target participants.  


Figure 4: Interactive plot with lines for eligible age for different clinical trail phases   
...
Figure 5: Unsurprisingly, the majority of recruiting studies for Alzheimer's Disease are geographically centered in Florida, a state known for a high proportion of elderly residents. Clicking on city markers will take the user to the ClinicalTrials.gov webpage for that particular study, allowing easy access to more information. 
