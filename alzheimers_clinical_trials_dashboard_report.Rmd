...
Interactive lineplot was created for those US studies with available age elligibility creteria, where a line represents a study, x-coordinates of a line (or length) represent study age elligibility, and y-coordinated of a line correspond to the number of subjects enrolled in a study.   
...
Figure 3: Unsurprisingly, the majority of recruiting studies for Alzheimer's Disease are geographically centered in Florida, a state known for a high proportion of elderly residents. Clicking on city markers will take the user to the ClinicalTrials.gov webpage for that particular study, allowing easy access to more information. 
