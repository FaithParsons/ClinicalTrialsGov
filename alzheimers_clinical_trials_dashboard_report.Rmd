...
>>>>>>> 0d46411cf61652d2dd3949c188760f7885a65a82
...
We also examined the number of new studies each year. Figure X1 shows that there were peaks in the number of new studies in 2009 and 2016. If we look closely in Figure X2, we can see that the spike in 2009 is due to the large increase in Phase 1 and/or Phase 2 studies, which could be indicative of a large number of new treatments being investigated. The spike in 2016, however, appears to be due to the combination of increased unclassified (in terms of study phase) trials, and a relatively high number of Phase 1/2 trials. 
...
us_studies %>%
  ...
  ggplot(aes(x = year_start, y = n)) + 
    ...
    labs(x = "Study start (year)", 
         y = "No. of studies", 
         title = "FIgure X1. No. of new Alzheimer's Disease studies per year") +
...
us_studies %>%
  ...
  ggplot(aes(x = year_start, y = n, color = type_phase)) + 
    ...
    labs(x = "Study start (year)", y = "No. of studies", 
         title = "Figure X2. No. of new Alzheimer's Disease studies per year by study type",
         color = "Study type") +
    scale_x_continuous(breaks = seq(1980, 2017, 5)) +
...
## Exploratory analysis [Volha will combine three figures and an interactive data table]   
...
Interactive lineplot was created for those US studies with available age elligibility creteria, where a line represents age elligibility for a particular study against the number of subjects enrolled in a study.   
...
The Gantt chart (**Figure 1**) and Age Eligibility plot (**Figure 2**) were created to visualize, for currently recruiting interventional phase 3 trials, how long each trial is expected to run, and the age range of target participants.   
...
Figure 3: Interactive map that allows users to visualize study locations based on user input specifications. Unsurprisingly, the majority of recruiting studies for Alzheimer's Disease are geographically centered in Florida, a state known for a high proportion of elderly residents. Clicking on city markers will take the user to the ClinicalTrials.gov webpage for that particular study, allowing easy access to more information. 
...
An interactive data table was created (**Table 1**, Appendix) to allow access to aditional information.   
